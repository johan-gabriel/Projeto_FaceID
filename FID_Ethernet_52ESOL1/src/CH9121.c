...
char stringConfig[] = "0;0;0;0;0;00;\n";
        ...
        while (uart_is_readable(UART_ID0))
        {
            ...
            if (i == 0)
            {
                ...
                if (resultado == 0xE6)
                {
                    ...
                    if ((comando[0] == '#') && (comando[1] == 'S') && (comando[2] == 'N') && (comando[3] == 'E') && (comando[4] != '\0') && (comando[5] != '\0')) // Comando: #SNE ( Set Número do Equipamento --> Parametramos de 01 a 99 ) Status: Implementado :)
                    {
...
void loop(void)
{
    ...
    if (((auxTemp >= 930) && (SSL[1] == 'f')) || ((auxTemp >= 1000) && (SSL[1] == 'n')) || ((auxTemp >= 1000) && (SSL[1] == '\0')))
    {
        if (tempo >= (auxTemp * 0.75))
        {
            sinaliza_fim_de_giro_proximo++;
            if ((sinaliza_fim_de_giro_proximo == 17) && (SSL[1] != '\0')) // Pisca Pictograma verde na Catraca ou Torniquete proximo ao final do Tempo
            {
                pisca_led_fim_de_giro = !pisca_led_fim_de_giro;
                sinaliza_fim_de_giro_proximo = 0;
            }

            if ((sinaliza_fim_de_giro_proximo == 18) && (SSL[1] == '\0')) // Pisca Pictograma verde na Porta proximo ao final do Tempo
            {
                pisca_led_fim_de_giro = !pisca_led_fim_de_giro;
                sinaliza_fim_de_giro_proximo = 0;
            }
    ...
    if ((inTemp >= Trpa * 93 && !trava_RPAn && (SSL[1] == 'f')) || (inTemp >= Trpa * 100 && !trava_RPAn && (SSL[1] == 'n')) || (inTemp >= Trpa * 100 && !trava_RPAn && (SSL[1] == '\0')))
    {
        auxTemp = inTemp;
        inTemp = 0;
        abre = false;
