...
char stringConfig[] = "0;0;0;0;0;0;1;0;0;2;0;01;01;\n";
...
void RX_TX()
{
    ...
    valorConfig4 = readSLE();
    valorConfig5 = readSNL();
    valorConfig6 = readSBZ();
    valorConfig7 = readSTL();
    valorConfig8 = readSQS();
    valorConfig9 = readSPX();
    ...
    valorSTE = readSTE();
    itoa(valorSTE, stringSTE, 10); // converte int em string
    dezena_STE = stringSTE[0];
    unidade_STE = stringConfig[1];
    printf("dez:%d, uni: %d", dezena_STE, unidade_STE);
    // printf("Valor SNL: %d , %d", SNL, valorConfig4);

    // dezena_STE = (int)readSNEdez();
    // unidade_STE =(int)readSNEuni();
    // printf("Valor SNEDez: %c",dezena_STE);
    // printf("Valor SNEUni: %c",unidade_STE);
    antSTE = dezena_STE;
    antSTEu = unidade_STE;
    ...
    while (stateConfig)
    {
            ...
            if (i == 0)
            {
                ...
                if (resultado == 0xD9)
                    if ((comando[0] == '#') && (comando[1] == 'H') && (comando[2] == 'E') && (comando[3] == 'L') && (comando[4] == 'P')) // Comando: #HELP Status ( Lista os comandos embarcados na placa ) Status: Implementado :)
                    {
                        ...
                        DEV_Delay_ms(40);
                        uart_puts(UART_ID1, "#GBZ : Get Buzzer (ligado / desligado)\n");
                        DEV_Delay_ms(40);
                        ...
                        DEV_Delay_ms(40);
                        uart_puts(UART_ID1, "#GQS : Get Quantidade de Solenoides\n");
                        DEV_Delay_ms(40);
                        ...
                        DEV_Delay_ms(40);
                        uart_puts(UART_ID1, "#SQS : Set Quant.de solenoides (1 ou 2)\n");
                        DEV_Delay_ms(40);
                ...
                if (resultado == 0xE3)
                {
                    if ((comando[0] == '#') && (comando[1] == 'G') && (comando[2] == 'B') && (comando[3] == 'Z')) // Comando: #GBZ ( Get Buzzer ) Status: Implementado :)
                    {
                        uart_puts(UART_ID1, "BUZo");
                        uart_putc(UART_ID1, SBZ);
                        if (SBZ == 'f')
                        {
                            uart_putc(UART_ID1, 'f');
                        }
                        zera_comando();
                        i = 0;
                    }
                ...
                if (resultado == 0xEB)
                    if ((comando[0] == '#') && (comando[1] == 'G') && (comando[2] == 'Q') && (comando[3] == 'S')) // Comando: #GQS ( Get Quantidade de Solenoides ) Status: Implementado :)
                    {
                        uart_puts(UART_ID1, "SQS");
                        uart_putc(UART_ID1, SQS);
                        zera_comando();
                        i = 0;
                    }
                ...
                if (resultado == 0xEC)
                {
                    ...
                    if ((comando[0] == '#') && (comando[1] == 'S') && (comando[2] == 'T') && (comando[3] == 'E') && ((comando[4] >= 48) && (comando[4] <= 57)) && ((comando[5] >= 48) && (comando[5] <= 57))) // Comando: #STE ( Set Tempo de Espera --> Passamos valores entre 01 a 99 ) Status: Implementado :)
                    {
                        {
                            if ((comando[4] != '\0') && (comando[5] != '\0'))
                            {
                            ...
                            {
                                dezena_STE = (STE / 10) + 48;
                                STE_return[0] = dezena_STE;

                                unidade_STE = (STE % 10) + 48;
                                STE_return[1] = unidade_STE;
                            }
                ...
                if (resultado == 0xEF)
                {
                    ...
                    if ((comando[0] == '#') && (comando[1] == 'S') && (comando[2] == 'B') && (comando[3] == 'Z') && (comando[4] == 'o') && ((comando[5] == 'n') || (comando[5] == 'f')) && ((comando[6] == '\0') || (comando[6] == 'f'))) // Comando: #SBZ ( Set Buzzer --> Passamos on ( ligado ) ou off ( desligado ) ) Status: Implementado :)
                    {
                        uart_puts(UART_ID1, "SBZok");
                        SBZ = comando[5];
                        zera_comando();
                        i = 0;
                    }
                ...
                if (resultado == 0xF7)
                {
                    if ((comando[0] == '#') && (comando[1] == 'S') && (comando[2] == 'Q') && (comando[3] == 'S') && ((comando[4] == '1') || (comando[4] == '2'))) // Comando: #SQS ( Set Quantidade de Solenoides --> Passamos: 1 ( 1 solenoide ) ou 2 ( 2 solenoides ) ) Status: Implementado :)
                    {
                        uart_puts(UART_ID1, "SQSok");
                        SQS = comando[4];
                        zera_comando();
                        i = 0;
                    }
        ...
        // novo
        if (STG != antSTG || SSL[1] != antSSL || SSH != antSSH || SPG != antSPG || dezena_SNE != antSNE || unidade_SNE != antSNEu || SFC != antSFC || SNL != antSNL || SPX != antSPX || SQS != antSQS || STL[0] != antSTL || SBZ != antSBZ || SLE[1] != antSLE || dezena_STE != antSTE || unidade_STE != antSTEu)
        {
            antSNL = SNL;
            antSSL = SSL[1];
            antSSH = SSH;
            ...
            if ((SSL[0] == 'o') && (SSL[1] == 'f') && (SSL[2] == 'f'))
            {
                stringConfig[4] = '0';
                storageLog(stringConfig);
            }
            if ((SSL[0] == 'o') && (SSL[1] == 'n') && (SSL[2] == '\0'))
            {
                stringConfig[4] = '1';
                storageLog(stringConfig);
            }
            ...
            if ((SLE[0] == 'o') && (SLE[1] == 'f') && (SLE[2] == 'f'))
            {
                stringConfig[10] = '0';
                storageLog(stringConfig);
            }
            if ((SLE[0] == 'o') && (SLE[1] == 'n') && (SLE[2] == '\0'))
            {
                stringConfig[10] = '1';
                storageLog(stringConfig);
            }
            ...
            stringConfig[22] = (char)dezena_SNE;
            stringConfig[23] = (char)unidade_SNE;
            storageLog(stringConfig);

            stringConfig[25] = (char)dezena_STE;
            stringConfig[26] = (char)unidade_STE;
            storageLog(stringConfig);
...
void loop(void)
{
    ...
    if (((auxTemp >= 2790) && (SSL[1] == 'f') && (SFC == 's')) || ((auxTemp >= 2800) && (SSL[1] == 'n') && (SFC == 's')) || ((auxTemp >= 5000) && (SFC == 'n')))
    {
    ...
    if ((inTemp >= Trpa * 279 && !trava_RPAn && (SSL[1] == 'f') && (SFC == 's')) || (inTemp >= Trpa * 280 && !trava_RPAn && (SSL[1] == 'n') && (SFC == 's')) || (inTemp >= Trpa * 500 && !trava_RPAn && (SFC == 'n')))
    {
        auxTemp = inTemp;
        inTemp = 0;
        abre = false;
